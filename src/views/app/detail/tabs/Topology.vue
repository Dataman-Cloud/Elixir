      ...
      <el-table-column type="expand">
        <template scope="prolong">
          <el-table v-if="prolong.row[1].length" :data="prolong.row[1]" border row-key="id" style="width: 100%">
            ...
            <el-table-column property="labels.VCLUSTER" label="所属集群" sortable>
            </el-table-column>
            <el-table-column property="instances" label="实例个数" sortable>
            </el-table-column>
                ...
                <el-tooltip class="item" effect="dark" placement="top">
                  <div slot="content">
                    <span v-for="(status, index) in app.row.healthData" :key="index" class="state">
                      状态:
                      <span :class="status.state"></span> {{status.state}} 数量: {{status.quantity}}
                      <br>
                    </span>
                  </div>
                  <ul class="progress">
                    <li v-for="(status, index) in app.row.healthData" :key="index" :class="status.state" :style="{width: (status.quantity / app.row.instances) * 100 + '%'}"></li>
                  </ul>
                </el-tooltip>
